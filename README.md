...
```env
# Django settings
SECRET_KEY=your_secret_key  # ⚠️ Замените на свой секретный ключ!
DEBUG=False
...
> 🎯 **Готово к использованию!** После запуска у вас будет полностью настроенный магазин с тестовыми данными.
...
- **Админ-панель**: http://localhost/admin (логин: admin@hopandbarley.com, пароль: admin123) ⚠️
- **API документация**: http://localhost/api/docs/

### Оптимизация Docker образа

Dockerfile оптимизирован для продакшена:
- Использует только необходимые зависимости из `requirements.txt`
- Автоматическая конвертация окончаний строк для Windows
- Очистка кэша apt для уменьшения размера образа
- Обновление pip для последней версии
...
└── requirements-dev.txt    # Зависимости для разработки (включает продакшен)
```

## 📋 Зависимости

### Продакшен
- `requirements.txt` - минимальные зависимости для продакшена (используется в Docker)

### Разработка
- `requirements-dev.txt` - включает продакшен + тестирование + линтинг
